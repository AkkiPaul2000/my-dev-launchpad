  ...
  return (
    <section id="Home" className="mx-auto grid max-w-container content-center gap-5 px-5 py-12 sm:px-8 lg:min-h-[720px] lg:grid-cols-12 lg:px-12 lg:py-16">
      <motion.div {...fadeUp(0.04)} className="relative overflow-hidden rounded-lg border border-border bg-card p-6 sm:p-9 lg:col-span-8 lg:p-12">
        ...
        <p className="mb-3 font-mono text-xs uppercase text-muted-foreground">Full Stack Developer · 4+ years</p>
      ...
      <motion.div {...fadeUp(0.16)} className="flex min-w-0 flex-col justify-between gap-6 rounded-lg border border-border bg-secondary p-6 lg:col-span-12 lg:flex-row lg:items-center">
        <div className="flex flex-wrap gap-2">
          {["TypeScript", "React", "NestJS", "Node.js", "AWS", "Docker", "Kubernetes"].map((skill) => <span key={skill} className="tech-chip">{skill}</span>)}
        </div>
        <a href="mailto:skypaul2000@gmail.com" className="inline-flex shrink-0 items-center gap-2 text-sm font-semibold text-foreground hover:text-primary">Let’s work together <ArrowUpRight size={17} /></a>
      </motion.div>
    </section>
  );
}
